...
RSpec.describe DataGenerator do
  ...
  def be_a_date_before_2000
    ...
    # to create a matcher that matches date before January 1st, 200
  ...
  def be_an_email_address
    ...
    match(/^[^@]+@(gmail\.com|yahoo\.com|aol\.com|hotmail\.com)$/)
  end
  ...
  def match_a_shape_of_an_user_record
    # Use `be_a_boolean`, `be_a_date_before_2000` and `be_an_email_address`
    # in the hash passed to `match` below to define this matcher.
    match(active: be_a_boolean, date_of_birth: be_a_date_before_2000, email_address: be_an_email_address)
  end

  it 'generates user records' do
    user = DataGenerator.new.user_record
    expect(user).to match_a_shape_of_an_user_record
  end

  def all_match_the_shape_of_an_user_record
    # Combine the `all` matcher and `match_the_shape_of_an_user_record` here.
    all(match_a_shape_of_an_user_record)
  end

  it 'generate a list of user records' do
    users = DataGenerator.new.users(4)
    expect(users).to all_match_the_shape_of_an_user_record
  end
end
